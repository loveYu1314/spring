...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	...
	//保存一个扫描指定类路径中Bean定义的扫描器，并将其设置到容器中
	 ...
	 */
	//默认构造函数，初始化一个空容器，容器不包含任何Bean信息，需要在稍后通过调用register()方法注册配置类，
	 ...
	 */
	//该构造函数会自动扫描给定的包及其子包下的所有类，并自动识别所有的Spring Bean，将其注册到容器中
	 ...
	 * @see #refresh()
	 */
	//为容器注册一个要被处理的注册Bean，新注册的Bean，必须手动调用容器的refresh()方法刷新容器，触发容器对新注册的Bean的处理
